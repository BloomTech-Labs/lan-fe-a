...
import { Layout, Menu, Breadcrumb, Input, Row, Col, Avatar } from 'antd';
import {
  UserOutlined,
  ShopFilled,
  LaptopOutlined,
  ...
  return (
    <Layout>
      <Header className="header" style={{ width: '100%' }}>
        {/* <div className="logo" /> */}
        <Row style={{ width: '100%', height: '100%', display: 'flex', justifyContent: 'space-between'}}>
          ...
          <Col span={6} style={{ display: 'flex', flexFlow: 'column nowrap', justifyContent: 'center'}}>
            <Search placeholder="input search text" onSearch={e => console.log('search implementation pending')} enterButton />
          </Col>
          <Col span={6} style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center'}} >
            <Avatar src={props.user.profilePicture} size="large"/>
            {/* <img src={props.user.profilePicture} alt='user avatar' /> */}
          </Col>
        </Row>
...
export default connect(mapStateToProps, { fetchRooms, fetchRecent, fetchUser })(SampleStyles);
